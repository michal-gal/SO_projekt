...
*/
  while (g->stolik_przydzielony == -1 && *restauracja_otwarta &&
    ...
    if (g->stolik_przydzielony == -1 && *restauracja_otwarta &&
        !prosba_zamkniecia) {
      sigsuspend(&old_set);
    }
  }
...
static void zamow_specjalne_jesli_trzeba(struct Grupa *g,
  ...
  for (int j = 0; j < stoliki[g->stolik_przydzielony].liczba_grup; j++) {
    if (stoliki[g->stolik_przydzielony].grupy[j].numer_grupy ==
        g->numer_grupy) {
...
sprobuj_pobrac_danie(struct Grupa *g, int *dania_pobrane, int dania_do_pobrania,
  ...
  if (clock_gettime(CLOCK_REALTIME, &ts) == 0) {
    ...
    (void)pthread_cond_timedwait(&tasma_sync->not_empty, &tasma_sync->mutex,
                                 &ts);
  }
...
static void opusc_stolik(const struct Grupa *g) {
  ...
  for (int j = 0; j < stoliki[g->stolik_przydzielony].liczba_grup; j++) {
    if (stoliki[g->stolik_przydzielony].grupy[j].numer_grupy ==
        g->numer_grupy) {
      for (int k = j; k < stoliki[g->stolik_przydzielony].liczba_grup - 1;
           k++) {
        stoliki[g->stolik_przydzielony].grupy[k] =
            stoliki[g->stolik_przydzielony].grupy[k + 1];
      }
      memset(&stoliki[g->stolik_przydzielony]
                  .grupy[stoliki[g->stolik_przydzielony].liczba_grup - 1],
