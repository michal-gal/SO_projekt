...
pid_t *pid_kierownik_shm; // wskaźnik na PID procesu kierownika w pamięci
                          // współdzielonej
...
void stworz_ipc(void) // tworzy zasoby IPC (pamięć współdzieloną i semafory)
{
  ...
  shm_id = shmget(IPC_PRIVATE, bufor_size,
                  IPC_CREAT | 0600); // utwórz pamięć współdzieloną
  void *pamiec_wspoldzielona =
      shmat(shm_id, NULL, 0); // dołącz pamięć współdzieloną
  ...
  kuchnia_dania_wydane =
      (int *)(tasma + MAX_TASMA); // kuchnia - liczba wydanych dań
  kasa_dania_sprzedane =
      kuchnia_dania_wydane + 6; // kasa - liczba sprzedanych dań
  restauracja_otwarta =
      kasa_dania_sprzedane + 6; // flaga czy restauracja jest otwarta
  ...
  klienci_opuscili =
      klienci_przyjeci + 1; // statystyka: klienci którzy opuścili

  pid_obsluga_shm =
      (pid_t *)(klienci_opuscili +
                1); // wskaźnik na PID procesu obsługi w pamięci współdzielonej
  pid_kierownik_shm =
      pid_obsluga_shm +
      1; // wskaźnik na PID procesu kierownika w pamięci współdzielonej
...
void kolejka_dodaj(struct Grupa g) // dodaje grupę do kolejki
{
  ...
  for (;;) {
    ...
    while (queue_sync->count >= queue_sync->max) {
      int rc = pthread_cond_timedwait(&queue_sync->not_full, &queue_sync->mutex,
                                      &abstime);
      if (rc == ETIMEDOUT) {
        if (!*restauracja_otwarta) {
...
struct Grupa kolejka_pobierz(void) // pobiera grupę z kolejki
{
  ...
  for (;;) {
    ...
    while (queue_sync->count <= 0) {
      ...
      int rc = pthread_cond_timedwait(&queue_sync->not_empty,
                                      &queue_sync->mutex, &abstime);
      if (rc == ETIMEDOUT) {
        if (!*restauracja_otwarta ||
            (shutdown_flag_ptr && *shutdown_flag_ptr)) {
          pthread_mutex_unlock(&queue_sync->mutex);
          return g;
        }
        clock_gettime(CLOCK_REALTIME, &now);
