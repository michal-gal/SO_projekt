...
if command -v setsid >/dev/null 2>&1; then
  setsid env RESTAURACJA_LOG_FILE="$LOG_FILE" RESTAURACJA_LOG_STDIO=0 RESTAURACJA_SEED=123 RESTAURACJA_CZAS_PRACY=30 ./build/bin/restauracja &
else
  env RESTAURACJA_LOG_FILE="$LOG_FILE" RESTAURACJA_LOG_STDIO=0 RESTAURACJA_SEED=123 RESTAURACJA_CZAS_PRACY=30 ./build/bin/restauracja &
fi
pid=$!
