...
int main(int argc, char **argv) {
  ...
  int czas_pracy =
      czas_pracy_domyslny; // domyślny czas pracy restauracji w sekundach
  ...
  // Przekaż shm_id/sem_id/msgq_id do procesów uruchamianych przez exec() jako
  // argumenty.
  snprintf(arg_shm, sizeof(arg_shm), "%d",
           shm_id); // przekazywanie jako string do exec()
  snprintf(arg_sem, sizeof(arg_sem), "%d",
           sem_id); // przekazywanie jako string do exec()
  snprintf(arg_msgq, sizeof(arg_msgq), "%d",
           msgq_id); // przekazywanie jako string do exec()

  *restauracja_otwarta = 1; // ustaw flagę otwarcia restauracji
  *kolej_podsumowania = 1;  // ustaw kolej na obsługę
  sygnalizuj_ture();

  pid_obsluga = uruchom_potomka_exec("./obsluga", "obsluga", 0,
                                     0); // uruchom proces obsługa
  if (pid_obsluga < 0)
  ...
  *pid_obsluga_shm = pid_obsluga;
  ...
  // Job-control sygnały z terminala (Ctrl+C / Ctrl+Z / Ctrl+\) trafiają do
  // grupy procesu rodzica. Ponieważ dzieci są w osobnej grupie, forwardujemy
  // je, żeby program reagował jak użytkownik oczekuje.
  ...
  pid_kucharz = uruchom_potomka_exec("./kucharz", "kucharz", 0,
                                     0); // uruchom proces kucharz
  if (pid_kucharz < 0)
    return awaryjne_zamkniecie_fork();
  if (children_pgid > 0)
    (void)setpgid(pid_kucharz, children_pgid);

  pid_kierownik = uruchom_potomka_exec("./kierownik", "kierownik", 0,
                                       0); // uruchom proces kierownik
  if (pid_kierownik < 0)
    return awaryjne_zamkniecie_fork();
  *pid_kierownik_shm = pid_kierownik;
  ...
  int max_aktywnych_klientow =
      MAX_AKTYWNYCH_KLIENTOW_DEFAULT; // limit aktywnych klientów naraz (można
                                      // nadpisać env)
  ...
  // symulacja pracy restauracji przez SIMULATION_SECONDS_DEFAULT sekund (bez
  // sleep)
  {
  ...
  int koniec_czasu = (elapsed_seconds_since(&start_czekania) >=
                      czas_pracy); // czy zakończenie z powodu upływu czasu
  int przerwano_sygnalem =
      sigint_requested; // czy zakończenie z powodu sygnału przerwania
  int zamknieto_flaga = (!koniec_czasu && !przerwano_sygnalem &&
                         !*restauracja_otwarta); // czy zakończenie z powodu
                                                 // zamknięcia restauracji flagą
  ...
  *kolej_podsumowania = 1;
  ...
  // Prosto: kończymy wszystkie dzieci jednym mechanizmem, niezależnie od typu
  // procesu.
